...
function Conversas() {
        ...
        document.querySelector('.message-content').addEventListener('mousewheel', (event) => {
            if (document.querySelector('.option-selected').id == content_id) {
                ...
                if ('wheelDelta' in event) {
                    ...
                    load.open("GET", "https://api.discord.berrytern.com.br:8082/conversa/" + `${id}/${before}/${next[1]}`)
    ...
    this.get = () => {
        ...
        request.open("GET", 'https://api.discord.berrytern.com.br:8082/conversas')
    ...
    this.addmsg = (messages, info_user, lastdivmsg = null, lastdivicon = null, lastid = null, nextid = null, nextnum = null, nextmsg = null, nexticon = null, before = false) => {
        ...
        for (let msgi in messages) {
            let msg
            if (before) {
                if (msgi == 0) { inverso = messages.length - 1 } else { inverso = inverso - 1 }; msg = inverso
            } else { msg = msgi }
...
let user_bar = () => {
    ...
    status.onmousemove = () => {
        if ((status.offsetTop + 12 > event.clientY && status.offsetLeft + 12 > event.clientX)) {
...
function change_status(status) {
    reauth()
    request = new XMLHttpRequest
    request.open('POST', "https://api.discord.berrytern.com.br:8082/status")
    request.setRequestHeader('Authorization', "Bearer " + user.token)
...
function user_edit(action = '') {
    if (action == 'edit') {
            ...
            if (!!password) {
                if (password.indexOf(' ') == -1) {
                    if (username != user.username) {
                        ...
                        u_change.open('POST', "https://api.discord.berrytern.com.br:8082/change/username")
                        ...
                        u_change.onload = () => {
                            if (u_change.status == 401) {
                                aviso(document.querySelector('.user-content'), 'Senha Incorreta', 'red', 3000);
                    ...
                    if ((user.icon) != document.querySelector('img.fileimg').src.split(',')[1] && !!document.querySelector('img.fileimg').src) {
                        ...
                        i_change.open('POST', "https://api.discord.berrytern.com.br:8082/change/icon", true)
                        ...
                        if (!!new_password) {
                            if (new_password.indexOf(' ') == -1) {
                                if (password == new_password) {
                                    aviso(document.querySelector('.user-content'), 'Senha antiga e nova são idênticas', 'red', 4000)
                                } else {
                                    p_change = new XMLHttpRequest
                                    p_change.open('POST', "https://api.discord.berrytern.com.br:8082/change/password")
        ...
        if (!!user.email) {
            document.querySelector('div.account-user.i .email').innerText = user.email
...
function Friends() {
    this.to = () => {
        ...
        if (f_topic == 'pedidos') {
            ...
            this.add()
        } else if (f_topic == 'bloqued') {
            this.bloqued()
        } else if (f_topic == 'able') {
            this.able()
        } else { this.all() }
    ...
    this.add = () => {
        ...
        btn.onclick = () => {
            ...
            pedid.onload = () => {
                ...
                } else if (pedid.status == 314) {
                    alert('pedido renviado')
                } else if (pedid.status == 312) {
        ...
        /*let request = new XMLHttpRequest
        request.open("POST", "https://api.discord.berrytern.com.br:8082"+"/friend/new")
        request.setRequestHeader('Authorization',"Bearer "+ user.token)
        request.send()
        request.onload= ()=>{
            let resp = JSON.parse(request.responseText)
    ...
    this.pedidos = () => {
        ...
        let request = new XMLHttpRequest
        request.open("POST", "https://api.discord.berrytern.com.br:8082" + "/pedidos")
        request.setRequestHeader('Authorization', "Bearer " + user.token)
        request.send()
        request.onload = () => {
            let resp = JSON.parse(request.responseText)
            ...
            if (resp.exist) {
                ...
                for (let friend in resp.list) {
                    ...
                    acc.onclick = () => {
                        ...
                        aceitar.open("POST", "https://api.discord.berrytern.com.br:8082/friend/new")
